...
export default function App() {
  ...
  const handlePair = async () => {
    if (!childId.trim()) {
      Alert.alert('Erreur', "Merci d'entrer le code fourni par le parent.");
      return;
    }
    if (!/^\d{4}$/.test(pin)) {
    ...
    await AsyncStorage.setItem(STORAGE_KEY_CHILD_ID, childId.trim());
      ...
      {!savedChildId ? (
        <View style={styles.card}>
          <Text style={styles.label}>
            Entrez le code d'appairage donné par votre parent
          </Text>
          <TextInput
            style={styles.input}
            value={childId}
          ...
          />
          <Text style={styles.label}>
            Code PIN parent (4 chiffres, à donner uniquement au parent)
          </Text>
          <TextInput
            style={styles.input}
            value={pin}
          ...
          <TouchableOpacity style={styles.button} onPress={handlePair}>
            <Text style={styles.buttonText}>Associer cet appareil</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <View style={styles.card}>
          <Text style={styles.label}>Appareil associé</Text>
